...
PIMercury663CalibrationDialog::PIMercury663CalibrationDialog(QWidget* parent, QFExtensionLinearStagePI663 *stage, int axis):
    QFDialog(parent)
{
    this->stage=stage;
    ...
    setWindowTitle(tr("PI Mercury C663 joystick calibration, stage %1").arg(axis));

    QTimer::singleShot(100, this, SLOT(getJoystickValues()));
}

PIMercury663CalibrationDialog::~PIMercury663CalibrationDialog()
{
    //dtor
}

void PIMercury663CalibrationDialog::resetCal() {
...
void PIMercury663CalibrationDialog::acceptCal() {
    QMutexLocker locker(serial->getMutex());
    accepted=true;
    // write calibration to controllers
...
void PIMercury663CalibrationDialog::calibrate() {
...
void PIMercury663CalibrationDialog::center() {
    QMutexLocker locker(serial->getMutex());
...
void PIMercury663CalibrationDialog::getJoystickValues() {
    QMutexLocker locker(serial->getMutex());
    if (com->isConnectionOpen()) {
        int jx=0, jy=0, jz=0;
        std::string r;
